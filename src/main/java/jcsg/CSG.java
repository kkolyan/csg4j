...
public class CSG {
    ...
    public static CSG sphere(SphereOptions options) {
        ...
        for (int i = 0; i < slices; i++) {
            for (int j = 0; j < stacks; j++) {
                ...
                local.vertex(i / slices, j / stacks);
                if (j > 0) local.vertex((i + 1) / slices, j / stacks);
                if (j < stacks - 1) local.vertex((i + 1) / slices, (j + 1) / stacks);
                local.vertex(i / slices, (j + 1) / stacks);
    ...
    public CSG cylinder(CylinderOptions options) {
        ...
        List<Polygon> polygons = new ArrayList<Polygon>();
        class Local {
            Vertex point(int stack, int slice, double normalBlend) {
        ...
        Local local = new Local();
        for (int i = 0; i < slices; i++) {
            int t0 = i / slices, t1 = (i + 1) / slices;
            polygons.add(new Polygon(Arrays.asList(start, local.point(0, t0, -1), local.point(0, t1, -1)), false));
            polygons.add(new Polygon(Arrays.asList(local.point(0, t1, 0), local.point(0, t0, 0), local.point(1, t0, 0), local.point(1, t1, 0)), false));
            polygons.add(new Polygon(Arrays.asList(end, local.point(1, t1, 1), local.point(1, t0, 1)), false));
        }
